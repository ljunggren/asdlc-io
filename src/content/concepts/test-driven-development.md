...
A single harness benchmark run (n=27 cases, 2026-06-29)[^1] tested a practical adaptation: **when existing tests already fail on the affected behavior, those tests are the RED phase — no new test is needed.** The agent skips test authorship and moves directly to fixing production code.
...
### When Agents Should Write New Tests

The fast path applies only when an existing test fails on the affected behavior. If no existing test catches the bug, the agent — or the human engineer governing the process — should write one first. This preserves TDD's design-pressure function for genuinely uncovered behavior.
